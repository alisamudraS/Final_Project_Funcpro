...
const ProfilAdmin = ({ route, navigation }) => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('pemilik') // Nama tabel di database Anda
        .select('nama_pemilik,password1')
  ...
  const handleEdit = (field) => {
    if (field === 'Username' || field === 'Password') {
      Alert.alert('Peringatan', `${field} tidak dapat diubah.`);
      return; // Menghentikan eksekusi jika field adalah username atau password
    }
  ...
  const handleSave = async () => {
    if (password !== userData.password1) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.profileHeader}>
        ...
        <Text style={styles.username}>{userData.nama_pemilik}</Text>
      </View>
      ...
      <View style={styles.inputContainer}>
        ...
        <TouchableOpacity onPress={() => handleEdit('Password')}>
          ...
          <TextInput
            style={styles.input}
            value={'*'.repeat(userData.password1.length)} // Masked password
            editable={false}
            pointerEvents="none"
          />
        </TouchableOpacity>
