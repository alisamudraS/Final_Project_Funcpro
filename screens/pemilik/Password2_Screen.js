  ...
  const handleAuth = useCallback(async () => {
    ...
    try {
      ...
      const { data: userPemilik, error: pemilikError } = await supabase
        ...
        .eq('password2', password)
        .single();

      if (userPemilik) {
        console.log(`Logged in as Pemilik, ID: ${userPemilik.id_pemilik}`);
        navigation.navigate('ScreenPassword3', { username : userPemilik.nama_pemilik, userId: userPemilik.id_pemilik });
        return;
      }
...
export default Password2_Screen;
