...
const ProfilScreen = ({ route, navigation }) => {
  ...
  const handleSave = async () => {
    ...
    // Handle duplicate check before saving
    const fieldToCheck = fieldMap[currentField];
    const checkDuplicate = await checkForDuplicate(fieldToCheck, newValue);

    if (checkDuplicate) {
      Alert.alert('Gagal', `${currentField} sudah terpakai.`);
      return;
    }
